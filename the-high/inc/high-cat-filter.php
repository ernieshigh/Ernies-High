...
function high_filter_cats() {
	...
	if( $cat_query->have_posts() ) : while ( $cat_query->have_posts() ) : $cat_query->the_post();
		...
		$dncats = get_the_terms( get_the_ID(), 'dummy_cat');
	...
	foreach($parent_ids as $parent_id){
		$parents[] = get_term($parent_id, 'dummy_cat'); 
	...
	foreach($cat_ids as $cat_id){
		$down_cats[] = get_term($cat_id, 'dummy_cat'); 
	}  
	...
	$old_parent = get_terms('dummy_cat', array('parent' => 0, 'orderby' => 'include','order' => 'ASC',  'hide_empty' => false)); 
	...
	foreach ($old_parent as $pterm){
		...
		$output .= '<li class="has-cats"><ul class="download-sub-cat">';
				...
				$terms = get_terms('dummy_cat', array('parent' => $termID, 'orderby' => 'slug', 'hide_empty' => false));
